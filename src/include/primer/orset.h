...
class ORSet {
 public:
  ...
  auto GetE()const ->std::unordered_map<T, std::set<int>>  { return e_set_; }
  auto GetT()const ->std::unordered_map<T, std::set<int>> { return t_set_; }

 private:
  // TODO(student): Add your private memeber variables to represent ORSet.
  std::unordered_map<T, std::set<int>> e_set_;
  std::unordered_map<T, std::set<int>> t_set_;
  // std::set<std::pair<T,int>>E_set; //current item
  // std::set<std::pair<T,int>>T_set; //delete item
};
